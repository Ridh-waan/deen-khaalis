...
import { nairobidata } from './Data';

const BASE_URL = 'https://www.deenkhaalis.com/'; // replace with your actual base URL
const ZiaraNairobi4 = () => {
    const [isplaying, setisplaying] = useState(false);
    const [currentSong, setCurrentSong] = useState(nairobidata[0]);
    const [songs, setSongs] = useState(nairobidata);
    const onPlaying = () => {
        ...
        <div className=''>
            <div className=' flex-col flex-grow overflow-y-auto'>
                ...
                <h2 className='text-center  text-2xl font-bold mb-5'>بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيم</h2>
                    <p className='text-center'>Hii ni mihadhara iliyotolewa
                        na sheikh wetu Abu Uthmaan Nurein, Al-Jabarty alipoenda ziara jijini Nairobi</p><br />
                    {songs.map((song, index) => (
                        <div className='py-10' key={index} onClick={() => selectSong(song)}>

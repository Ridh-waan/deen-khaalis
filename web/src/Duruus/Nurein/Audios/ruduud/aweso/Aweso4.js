...
import { awesodata } from './Data';

const BASE_URL = 'https://www.deenkhaalis.com/'; // replace with your actual base URL
const Aweso4 = () => {
    const [isplaying, setisplaying] = useState(false);
    const [currentSong, setCurrentSong] = useState(awesodata[0]);
    const [songs, setSongs] = useState(awesodata);
    const onPlaying = () => {
        ...
        <div className=''>
            <div className=' flex-col flex-grow overflow-y-auto'>
                ...
                <h2 className='text-center  text-2xl font-bold mb-5'>بِسۡمِ مِٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيم</h2>
                    <p className='text-center'>Hii ni raddi Kuhusiana na Kadhia ya Lamu na Dawrah Iliyodaiwa
                        iliyotolewa na sheikh wetu Abu Uthmaan Nurein Al-Jabarty</p><br />
                    {songs.map((song, index) => (
                        <div className='py-3' key={index} onClick={() => selectSong(song)}>

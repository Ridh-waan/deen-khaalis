...
import { nafsidata } from './Data';

const BASE_URL = 'https://www.deenkhaalis.com/'; // replace with your actual base URL
const KuitaasaNafsi4 = () => {
    const [isplaying, setisplaying] = useState(false);
    const [currentSong, setCurrentSong] = useState(nafsidata[0]);
    const [songs, setSongs] = useState(nafsidata);
    const onPlaying = () => {
        ...
        <div className=''>
            <div className=' flex-col flex-grow overflow-y-auto'>
                <div className=" py-7 px-8 mt-20 cursor-pointer mb-4">
                    ...
                    <p className='text-center'>Hii ni misingi kumi ya kuitakasa nafsi iliyofundishwa
                        na sheikh wetu Abu Uthmaan Nurein, Al-Jabarty</p><br />
                    {songs.map((song, index) => (
                        <div className='py-1' key={index} onClick={() => selectSong(song)}>

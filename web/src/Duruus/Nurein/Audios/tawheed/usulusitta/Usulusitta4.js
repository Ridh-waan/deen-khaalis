...
import { usulusittadata } from './Data';

const BASE_URL = 'https://www.deenkhaalis.com/'; // replace with your actual base URL
const Usulusitta4 = () => {
    const [isplaying, setisplaying] = useState(false);
    const [currentSong, setCurrentSong] = useState(usulusittadata[0]);
    const [songs, setSongs] = useState(usulusittadata);
    const onPlaying = () => {
        ...
        <div className=''>
            <div className=' flex-col flex-grow overflow-y-auto'>
                ...
                <h2 className='text-center  text-2xl font-bold mb-5'>بِسۡمِ مِٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيم</h2>
                    <p className='text-center'>Hizi ni sauti za risala ya Usulusitta chake
                        Sheikh Muhhammad ibn Abdulwahaab zilizotolewa na sheikh wetu Abu Uthmaan Nurein Al-Jabarty</p><br />
                    {songs.map((song, index) => (
                        <div className='py-32' key={index} onClick={() => selectSong(song)}>

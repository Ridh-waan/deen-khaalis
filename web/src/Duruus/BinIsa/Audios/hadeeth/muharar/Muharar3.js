...
import { muharardata } from './Data';

const BASE_URL = 'https://www.deenkhaalis.com/'; // replace with your actual base URL
const Muharar3 = () => {
    const [isplaying, setisplaying] = useState(false);
    const [currentSong, setCurrentSong] = useState(muharardata[0]);
    const [songs, setSongs] = useState(muharardata);
    const onPlaying = () => {
        ...
        <div className=''>
            <div className=' flex-col flex-grow overflow-y-auto'>
                <div className=" py-7 px-8 mt-20 cursor-pointer mb-4">
                    <h2 className='text-center  text-2xl font-bold mb-5'>بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ</h2>
                    <p className='text-center'>Hizi ni sauti za kitabu Al Muharar fil Hadeeth cha Sheikh
                        Ibn Abdilhaady
                        zilizotolewa na sheikh wetu Abu Issa Muhhamad Bin Issa </p><br />
                    {songs.map((song, index) => (
                        <div className='py-6' key={index} onClick={() => selectSong(song)}>
